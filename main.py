...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are BUDDY, Buddy's AI assistant. "
...
class BuddyLive:
    ...
    def _on_text_command(self, text: str):
        if not self._loop or not self.session:
            return
        from core.interrupt import check_for_interrupt_words, set_interrupt
        if check_for_interrupt_words(text):
            set_interrupt(True)
            self.speak("Okay! I've stopped the current task. What would you like me to do instead?")
            self.ui.write_log("SYS: User interrupted via text.")
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            result = "Task was interrupted by the user."
            print(f"[BUDDY] 🛑 Tool '{name}' interrupted.")
            ...
            while True:
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.input_transcription and sc.input_transcription.text:
                            ...
                            if txt:
                                in_buf.append(txt)
                                from core.interrupt import check_for_interrupt_words, set_interrupt
                                if check_for_interrupt_words(txt):
                                    set_interrupt(True)
                                    self.speak("Okay! I've stopped the current task. What would you like me to do instead?")
                                    self.ui.write_log("SYS: User interrupted via voice.")
                                    self.ui.set_state("LISTENING")
                                    in_buf = []
                    ...
                    if response.tool_call:
                        from core.interrupt import set_interrupt, is_interrupted
                        set_interrupt(False)
                        fn_responses = []
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        Two-phase briefing for instant perceived response:
          Phase 1 — immediate greeting (no tools, no fetch) → Buddy speaks in <2s
