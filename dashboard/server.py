"""
dashboard/server.py — BUDDY Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "BUDDY Uploads",
        Path.home() / "Documents" / "BUDDY Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'BUDDY-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"BUDDY Dashboard Port {port}"
        prog_rule  = "BUDDY Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="buddy_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until BUDDY is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "buddy.key").exists() and (certs / "buddy.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in BUDDY to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('buddy_token','{tok}');
  sessionStorage.setItem('buddy_key','{key}');
  localStorage.setItem('buddy_device_token','{dev_tok}');
...
<p>Connecting to BUDDY…</p>
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "buddy.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "buddy.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in BUDDY UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
